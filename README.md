...
**v0.1.6–v0.1.9 点「下载并安装」会永远停在「正在安装」。** 那几版移交后没能真的退出应用（原因见「在线更新实现要点」的「主进程怎么退」），更新助手等满 60s 就放弃替换。此时更新已经中止，而且这个面板会连 ⌘Q 一起挡掉——用「强制退出」（`⌥⌘Esc`）结束应用，再手动装 v0.1.10 或更新的版本，之后的更新就都能在应用内走完。
